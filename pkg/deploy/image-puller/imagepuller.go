...
func ReconcileImagePuller(ctx *chetypes.DeployContext) (reconcile.Result, bool, error) {
	...
	if ctx.CheCluster.Spec.ServerComponents.ImagePuller.Enable && (!foundPackagesAPI || !foundOperatorsAPI) {
	...
	if ctx.CheCluster.Spec.ServerComponents.ImagePuller.Enable {
		...
		if foundKubernetesImagePullerAPI {
			...
			if ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec.DeploymentName == "" {
				ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec.DeploymentName = imagePuller.Spec.DeploymentName
			}
			if ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec.ConfigMapName == "" {
				ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec.ConfigMapName = imagePuller.Spec.ConfigMapName
			}
			if ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec.ImagePullerImage == "" {
				ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec.ImagePullerImage = imagePuller.Spec.ImagePullerImage
			}

			// If ImagePuller specs are different, update the KubernetesImagePuller CR
			if imagePuller.Spec != ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec {
				imagePuller.Spec = ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec
...
func UpdateImagePullerSpecIfEmpty(ctx *chetypes.DeployContext) (chev2.CheClusterSpecImagePuller, error) {
	...
	if ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec.ConfigMapName == "" {
		ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec.ConfigMapName = "k8s-image-puller"
	}
	err := ctx.ClusterAPI.Client.Update(context.TODO(), ctx.CheCluster, &client.UpdateOptions{})
	if err != nil {
		return ctx.CheCluster.Spec.ServerComponents.ImagePuller, err
	}
	return ctx.CheCluster.Spec.ServerComponents.ImagePuller, nil
}
...
func GetExpectedKubernetesImagePuller(ctx *chetypes.DeployContext) *chev1alpha1.KubernetesImagePuller {
	return &chev1alpha1.KubernetesImagePuller{
		...
		Spec: ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec,
	}
}
...
func UpdateDefaultImagesIfNeeded(ctx *chetypes.DeployContext) error {
	specImages := StringToImageSlice(ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec.Images)
	defaultImages := GetDefaultImages()
...
func SetImages(ctx *chetypes.DeployContext, images []ImageAndName) error {
	...
	ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec.Images = imagesStr
...
func UninstallImagePullerOperator(ctx *chetypes.DeployContext) (bool, error) {
	...
	if (ctx.CheCluster.Spec.ServerComponents.ImagePuller.Enable || ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec != (chev1alpha1.KubernetesImagePullerSpec{})) &&
		ctx.CheCluster.ObjectMeta.DeletionTimestamp.IsZero() {
		ctx.CheCluster.Spec.ServerComponents.ImagePuller.Spec = chev1alpha1.KubernetesImagePullerSpec{}
