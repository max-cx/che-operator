...
func (s CheServerReconciler) getDeploymentSpec(ctx *chetypes.DeployContext) (*appsv1.Deployment, error) {
	...
	if ctx.CheCluster.Spec.Workspaces.TrustedCerts.GitTrustedCertsConfigMapName != "" {
		gitSelfSignedCertEnv = corev1.EnvVar{
			Name: "CHE_GIT_SELF__SIGNED__CERT",
			ValueFrom: &corev1.EnvVarSource{
				ConfigMapKeyRef: &corev1.ConfigMapKeySelector{
					Key: "ca.crt",
					LocalObjectReference: corev1.LocalObjectReference{
						Name: ctx.CheCluster.Spec.Workspaces.TrustedCerts.GitTrustedCertsConfigMapName,
					},
					Optional: &optionalEnv,
				},
			},
	...
	chePostgresCredentialsSecret := utils.GetValue(ctx.CheCluster.Spec.ServerComponents.Database.CredentialsSecretName, constants.DefaultPostgresCredentialsSecret)
	...
	if defaults.IsComponentReadinessInitContainersConfigured() {
		if !ctx.CheCluster.Spec.ServerComponents.Database.ExternalDb {
	...
	deploy.CustomizeDeployment(deployment, &ctx.CheCluster.Spec.ServerComponents.CheServer.Deployment, true)
