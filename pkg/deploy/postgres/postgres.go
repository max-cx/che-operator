...
func (p *PostgresReconciler) syncPVC(ctx *chetypes.DeployContext) (bool, error) {
	pvc := ctx.CheCluster.Spec.ServerComponents.Database.Pvc.DeepCopy()
	pvc.ClaimSize = utils.GetValue(ctx.CheCluster.Spec.ServerComponents.Database.Pvc.ClaimSize, constants.DefaultPostgresPvcClaimSize)
...
func (p *PostgresReconciler) syncCredentials(ctx *chetypes.DeployContext) (bool, error) {
	postgresCredentialsSecretName := utils.GetValue(ctx.CheCluster.Spec.ServerComponents.Database.CredentialsSecretName, constants.DefaultPostgresCredentialsSecret)
