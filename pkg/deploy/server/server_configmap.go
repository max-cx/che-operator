...
func (s *CheServerReconciler) getCheConfigMapData(ctx *chetypes.DeployContext) (cheEnv map[string]string, err error) {
	...
	if ctx.CheCluster.Spec.ServerComponents.CheServer.Deployment.SecurityContext.FsGroup != nil {
		securityContextFsGroup = strconv.FormatInt(*ctx.CheCluster.Spec.ServerComponents.CheServer.Deployment.SecurityContext.FsGroup, 10)
	}

	securityContextRunAsUser := strconv.FormatInt(constants.DefaultSecurityContextRunAsUser, 10)
	if ctx.CheCluster.Spec.ServerComponents.CheServer.Deployment.SecurityContext.RunAsUser != nil {
		securityContextRunAsUser = strconv.FormatInt(*ctx.CheCluster.Spec.ServerComponents.CheServer.Deployment.SecurityContext.RunAsUser, 10)
	}
	...
	chePostgresHostName := utils.GetValue(ctx.CheCluster.Spec.ServerComponents.Database.PostgresHostName, constants.DefaultPostgresHostName)
	chePostgresPort := utils.GetValue(ctx.CheCluster.Spec.ServerComponents.Database.PostgresPort, constants.DefaultPostgresPort)
	chePostgresDb := utils.GetValue(ctx.CheCluster.Spec.ServerComponents.Database.PostgresDb, constants.DefaultPostgresDb)
	...
	for _, r := range ctx.CheCluster.Spec.ServerComponents.DevfileRegistry.ExternalDevfileRegistries {
	...
	cheLogLevel := utils.GetValue(ctx.CheCluster.Spec.ServerComponents.CheServer.LogLevel, constants.DefaultServerLogLevel)
	cheDebug := strconv.FormatBool(ctx.CheCluster.Spec.ServerComponents.CheServer.Debug)
	cheMetrics := strconv.FormatBool(ctx.CheCluster.Spec.ServerComponents.Metrics.Enable)
	...
	if !ctx.CheCluster.Spec.ServerComponents.PluginRegistry.DisableInternalRegistry {
	...
	addMap(cheEnv, ctx.CheCluster.Spec.ServerComponents.ExtraProperties)
