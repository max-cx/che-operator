...
func (c *CheCluster) IsImagePullerSpecEmpty() bool {
	return c.Spec.ServerComponents.ImagePuller.Spec == (imagepullerv1alpha1.KubernetesImagePullerSpec{})
}

func (c *CheCluster) IsImagePullerImagesEmpty() bool {
	return len(c.Spec.ServerComponents.ImagePuller.Spec.Images) == 0
}

func (c *CheCluster) GetBaseDomain() string {
	if c.Status.WorkspaceBaseDomain != "" {
		return c.Status.WorkspaceBaseDomain
	}

	if infrastructure.IsOpenShift() {
		return c.Spec.ServerComponents.ExtraProperties["CHE_INFRA_OPENSHIFT_ROUTE_HOST_DOMAIN__SUFFIX"]
	}

	return c.Spec.Ingress.Domain
}
...
func (c *CheCluster) GetDefaultNamespache() string {
	if c.Spec.ServerComponents.ExtraProperties != nil {
		k8sDefaultNamespace := c.Spec.ServerComponents.ExtraProperties["CHE_INFRA_KUBERNETES_NAMESPACE_DEFAULT"]
		if k8sDefaultNamespace != "" {
			return k8sDefaultNamespace
		}
	}

	if c.Spec.Workspaces.DefaultNamespace != "" {
		return c.Spec.Workspaces.DefaultNamespace
	}

	return "<username>-" + os.Getenv("CHE_FLAVOR")
}

func (c *CheCluster) IsMultiUser() bool {
	return c.Spec.ServerComponents.ExtraProperties == nil || c.Spec.ServerComponents.ExtraProperties["CHE_MULTIUSER"] != "false"
}
