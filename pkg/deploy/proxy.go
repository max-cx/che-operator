...
func ReadCheClusterProxyConfiguration(ctx *chetypes.DeployContext) (*chetypes.Proxy, error) {
	proxyParts := strings.Split(ctx.CheCluster.Spec.ServerComponents.CheServer.Proxy.Url, "://")
	proxyProtocol := ""
	proxyHost := ""
	if len(proxyParts) == 1 {
		proxyProtocol = ""
	...
	proxyURL := proxyHost
	if ctx.CheCluster.Spec.ServerComponents.CheServer.Proxy.Port != "" {
		proxyURL = proxyURL + ":" + ctx.CheCluster.Spec.ServerComponents.CheServer.Proxy.Port
	}

	proxyUser := ""
	proxyPassword := ""

	proxyCredentialsSecretName := utils.GetValue(ctx.CheCluster.Spec.ServerComponents.CheServer.Proxy.CredentialsSecretName, constants.DefaultProxyCredentialsSecret)
	...
	return &chetypes.Proxy{
		...
		HttpUser:     proxyUser,
		HttpHost:     proxyHost,
		HttpPort:     ctx.CheCluster.Spec.ServerComponents.CheServer.Proxy.Port,
		HttpPassword: proxyPassword,

		HttpsProxy:    proxyURL,
		HttpsUser:     proxyUser,
		HttpsHost:     proxyHost,
		HttpsPort:     ctx.CheCluster.Spec.ServerComponents.CheServer.Proxy.Port,
		HttpsPassword: proxyPassword,

		NoProxy: strings.Join(ctx.CheCluster.Spec.ServerComponents.CheServer.Proxy.NonProxyHosts, ","),
	}, nil
}
